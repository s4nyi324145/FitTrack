...
const ExerciseDetailModal = ({ exercise, onClose }: Props) => {
  const muscle =
    muscleGroupColors[exercise.muscle_group] ?? muscleGroupColors.full_body;
  const equip = equipmentConfig[exercise.equipment] ?? equipmentConfig.other;
  const tipsArray = exercise.tips
    ?.split("-")
    .map((t) => t.trim())
    .filter(Boolean);
      ...
      >
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={onClose}
              className="p-1.5 cursor-pointer flex items-center justify-center rounded-full bg-white/20 hover:bg-white/30 text-white transition-colors"
            >
      ...
      <div className="flex flex-col md:flex-row gap-6 p-6">
        ...
        <div className="flex flex-col gap-3 flex-[0.45]">
          ...
          {[
            ...
            { label: "Est. 1RM", sub: "Epley formula", value: "—", unit: "kg" },
          ].map((pr) => (
            <div
              key={pr.label}
            ...
            >
              <div className="flex flex-col gap-0.5">
                <p className="text-xs font-bold text-foreground">{pr.label}</p>
                <p className="text-xs text-text-muted">{pr.sub}</p>
              </div>
