...
export const uploadExercise = async (newExercise: CreateExerciseInput) => {
  ...
  try {
    const result = await pool.query(
      "INSERT INTO exercises (user_id,name,muscle_group,equipment,exercise_type,instructions,is_custom) VALUES ($1,$2,$3,$4,$5,$6, true) RETURNING id",
      [
        userId,
        newExercise.name,
    ...
    return { succes: true, newExercise: result.rows[0].id };
  } catch (error) {
