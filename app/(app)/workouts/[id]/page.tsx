...
import ExercisesBreakdown from '@/components/detailedWorkout/ExercisesBreakdown';
import DeleteWorkoutButton from '@/components/detailedWorkout/DeleteWorkoutButton';
import EditWorkoutButton from '@/components/detailedWorkout/EditWorkoutButton';
...
export default async function WorkoutDetailPage({ params }: WorkoutsDetailPageProps) {
  ...
  const duration = workoutDetail.duration
    ? workoutDetail.duration / 60
    : "-"

  

  const stats = [
    { icon: <Weight size={18} className="text-purple-500" />,  iconBg: "bg-purple-500/10", label: "Total Volume",  value: workoutDetail.volume ? `${workoutDetail.volume} kg` : "—" },
    { icon: <Dumbbell size={18} className="text-blue-500" />,  iconBg: "bg-blue-500/10",   label: "Exercises",    value: workoutDetail.exercises.length },
    { icon: <Layers size={18} className="text-primary" />,     iconBg: "bg-primary/10",    label: "Total Sets",   value: workoutDetail.total_sets },
    { icon: <Clock size={18} className="text-orange-500" />,   iconBg: "bg-orange-500/10", label: "Duration",     value: duration ? `${duration} min` : "—" },
  ]
      ...
      <div className="flex items-start justify-between">
        <div className="flex gap-1 flex-col">
          ...
          <p className="text-text-muted flex gap-2   text-sm">
            ...
            {duration && <span className='flex items-center gap-2'> · <Clock size={14}/> {duration} min</span>}
          </p>
        </div>
